...
\frame[t] {
	...
	to the generation of a color palette from another on base a one set of example transformations.

	\medskip

	In this scenario we have a palette, a set of colors, and a
	bunch of examples of how to change them to another palette.

	\medskip

	Each example takes a color, specified by its components	(\textit{red}, \textit{green}, \textit{blue}) let's name it
	$u$ (after \textit{u}rigin) and produces anoter color $v$ (from \textit{v}estiny). So
...
\frame[t] {
	\frametitle{Least squares}
	\framesubtitle{Error en el tramo 20:32-20:37}

	Un polinomio en notación sumatoria se expresa:
	$
	\sum_{i=0}^{n} a_ix^i  
	$
	que es la suma del coeficiente por la potencia de la indeterminada
	...
	Un polinomio cúbico: $ a_3x^3+a_2x^2+a_1x^1+a_0x^0 $
	
	Un polinomio cuadrático: $ a_2x^2+a_1x^1+a_0x^0 $
	
	Un polinomio lineal: $a_1x^1+a_0x^0 $

	En vez de $x^1$ decimos $x$ y en vez de $x^0$ decimos $1$
	Como $a_0.1 = a_0$ no lo escribimos y decimos que el polinomio
	lineal es $a_1x+a_0$  
	
	\medskip

	Pero, cuando lo pasamos a forma vectorial:
